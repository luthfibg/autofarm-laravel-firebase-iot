...
@section('content-title')
    <p style="font-size: 12pt">{{ __('Realtime Data') }} <span> <i>{{ __('Refresh to show current data') }}</i> </span><p>
@endsection

@section('content')
    <div class="lg:flex lg:items-center lg:justify-between">
        <div class="min-w-0 flex-1">
          <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">{{ __('Humidity Data Monitor') }}</h2>
        ...
        <div class="mt-5 flex lg:mt-0 lg:ml-4">
          ...
          <div class="relative ml-3 sm:hidden">
            ...
            <div class="absolute right-0 z-10 mt-2 -mr-1 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="mobile-menu-button" tabindex="-1">
              ...
              <a href="#" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="mobile-menu-item-0">{{ __('Edit') }}</a>
              <a href="#" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="mobile-menu-item-1">{{ __('View') }}</a>
            </div>
          </div>
        </div>
    </div>
    <div class="overflow-hidden bg-white shadow sm:rounded-lg mt-3">
        <div class="px-4 py-5 sm:px-6">
          <h3 class="text-lg font-medium leading-6 text-gray-900">{{ __('Kondisi Lingkungan') }}</h3>
          <p class="mt-1 max-w-2xl text-sm text-gray-500">{{ __('Pantauan kondisi lingkungan terbaru') }}</p>
        </div>
        <div class="border-t border-gray-200">
          <dl>
            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium text-gray-500">{{ __('Kelembapan tanah (humidity soil)') }}</dt>
